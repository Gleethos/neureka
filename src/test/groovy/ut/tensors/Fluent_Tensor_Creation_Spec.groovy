...
class Fluent_Tensor_Creation_Spec extends Specification
{
    ...
    def 'Seed based tensors can be created fluently.'(
        ...
        where : 'The following data is being used to populate the builder API:'
            type     | seed || expected
            Integer  | "a"  || [-272402222, 109653436, -561394618, -572023092, -924061463, 683685334] as int[]
            ...
            Byte     | "e"  || [-122, 112, -7, -128, -99, -118] as byte[]
            Long     | "f"  || [-1161696212425817999, 479220343296026873, -2402909108810538396, -2448558049951123424, -3960551339440580624, 2944668569588759120] as long[]
