...
class Eleven_Lines_NN_System_Spec extends Specification {
    ...
    def 'One can write a simple neural network with custom back-prop in 11 lines of code!'()
    {
        ...
        expect :
            W1.mut.data.get().collect({it.round 14}) == RESULT_W1.collect({it.round 14})
            W2.mut.data.get().collect({it.round 14}) == RESULT_W2.collect({it.round 14})
    }
    ...
    def 'One can write a simple float based neural network in less than 11 lines of java like code!'()
    {
        ...
        expect :
            W1.mut.data.get() == RESULT_W1_F32 as float[]
            W2.mut.data.get() == RESULT_W2_F32 as float[]
    }
    ...
    def 'One can write a simple double based neural network in less than 11 lines of java like code using the "@" operator!'()
    {
        ...
        expect :
            W1.mut.data.get().collect({it.round 12}) == RESULT_W1.collect({it.round 12})
            W2.mut.data.get().collect({it.round 12}) == RESULT_W2.collect({it.round 12})
    }
    ...
    def 'The pseudo random number generator works as expected for the weights used in the 11 line NN examples!'()
    {
        ...
        expect :
            ...
            W2.mut.data.get() == [-0.1611330910958405, -0.9350019667613545, -0.3780200880067806, 1.4951759768158595]
    }
