...
class Tensor_Operation_Component_Tests extends Specification{
    ...
    def 'Operators "+,*,**,^" produce expected results with gradients which can be accessed via a "Ig[0]" Function instance'()
    {
        ...
        then : FunctionBuilder.build("Ig[0]", false)(trs).toString().equals("[1]:(-8.0)")

        when : trs[0] = y
        then : assert FunctionBuilder.build("Ig[0]", false)(trs).toString().contains("[1]:(4.0); ->d[1]:(-8.0)")
    }
