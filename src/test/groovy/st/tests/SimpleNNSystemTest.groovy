...
class SimpleNNSystemTest
{
    ...
    SimpleNNSystemTest(Mode mode) {
        ...
        switch (mode) {
            case Mode.CONVOLUTION:
                _$   = { a, b -> Tensor.of('I[0] x I[1]', [a, b]) }
                back = { a, b -> Tensor.of('I[0] x I[1]', [a, b]) }
                back2 = { a, b -> Tensor.of('I[0] x I[1]', [a, b]) }; break
            case Mode.MAT_MUL:
                _$    = { a, b -> a.matMul(b) }
    ...
    void on(Device device)
    {
        ...
        var X = Tensor.of( // input data: 5 vectors in binary form
                                inputShape, // (5x3)
                                [
                                    0d, 0d, 1d,
                                    1d, 1d, 0d,
        ...
        var y = Tensor.of(outputShape,[0d, 1d, 1d, 1d, 0d]).to(device)

        var input = X
        var weights1 = Tensor.of(w1Shape,
        ...
        */
        var weights2 = Tensor.of(w2Shape, [0.20445225d, 0.87811744d, 0.02738759d, 0.67046751d]).to(device)
        /*
            [1x1x4]:...
            w2 (4, 1) :
            [[0.20445225]
             ...
             [0.67046751]]
         */
        var output = Tensor.of(outputShape, [0d, 0d, 0d, 0d, 0d]).to(device)
        /*
            [5x1x1]...
            out (5, 1) :
            [[0.0]
             ...
             [0.0]]
        */
        var layer1 = Tensor.of(layer1Shape, 0d).to(device)
    ...
    static Tensor sigmoid(Tensor x) {
        return Tensor.of("sig(I[0])", x)
        // Other possibilities:
        //return Tensor.of(((Tensor.Create.E(x.shape())**(-x))+1), "1/I[0]")
        //return 1.0 / (1 + Tensor.Create.E(x.shape())**(-x))
    }

    static Tensor sigmoid_derivative(Tensor x) {
        return x * (-x + 1)
    }

    void feedforward(Tensor weights1, Tensor weights2, Tensor input, Tensor output, Tensor layer1) {
    ...
    void backprop(Tensor weights1, Tensor weights2, Tensor input, Tensor output, Tensor layer1, Tensor y) {
        // application of the chain rule to find derivative of the loss function with respect to weights2 and weights1
        Tensor delta = (y - output) * 2
        Tensor derivative = delta * 2 * sigmoid_derivative(output)
        Tensor d_weights2 = back(layer1, derivative)
        Tensor temp = back2(derivative, weights2)
        Tensor d_weights1 = back(input, temp * sigmoid_derivative(layer1))
