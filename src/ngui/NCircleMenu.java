...
public class NCircleMenu implements NPanelObject{
	...
	(int id, Object thing)->
	{
		System.out.println("Actor has not been set to do anything :/ ");
	};
	...
	{
		System.out.println("set layerID...");
		layerID=id;
	}
	...
	public ArrayList<NPanelRepaintSpace> updateOn(NPanelAPI HostPanel)
	{
		...
		if(needsRepaint==true) 
		{
		       ...
		       queue.add(new NPanelRepaintSpace(X, Y, (int)scaledRadius, (int)scaledRadius));//  queue.add(new NPanelRepaintSpace(X, Y, (int)radius, (int)radius));
		       //if(X!=oldX || Y!=oldY) {queue.add(new NPanelRepaintSpace(oldX, oldY, (int)scaledRadius, (int)scaledRadius)); oldX=X; oldY=Y;}
		}
		else 
