...
public class NGraphBuilder {
	...
	private void addPanelNeuron(int x, int y)       
 	...
 	private void addPanelNeuron(ngui.NPanelNode PanelNeuron) 
 	{
	...
	{	
		Surface = new NPanel();
		NPanelNode Neuron = new ngui.NPanelNode(-400, 2500, 0);
	 	NVertex.State.turnIntoParentRoot(Neuron.getCore()); // turnIntoSuperRootNode();
	 
	 	addPanelNeuron(Neuron);
	 	NPanelNode Neuron1 = new ngui.NPanelNode( 700, -2600, 1 );
	 	...
	 	NPanelNode Neuron4 = new ngui.NPanelNode( 3090, 890, 2 );
	...
	//-------------------------------------------------------------------------
	 public void setHistoryScopeTo(int scope) 
	 {
	 	...
	 	(NPanelObject node)->
	 	{
	 		if(!(node instanceof ngui.NPanelNode)) {return false;}
