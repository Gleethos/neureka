...
public class NVConvector extends Kernel 
{
    ...
    int[] Anchor_Arowlayer;// Defines anchor points on the row layer of A.
    
    int[] Bdim;//Bcomp1[d1,d2,d3...]Bcomp2[d1,d2,d3]
    int[] Bdim_anchor;
    
    int[] Cdim;
    //Does not need an anchor. only represents one vertex!
    
    boolean[] inversed = {false};
  //====================================================
    public int convectionSize() {if(inversed[0]) {return collom*common;}else {return Anchor_AoBcomp.length*collom*row;}}
    
    public void setInversion(boolean invers) {inversed[0]=invers;}
    ...
    //==========================================================================
    public NVConvector(int row, int collom, int[] Bcomposition, int[] A_Assoc_Composition)
    {
    ...
    public void printA()
    {
    	...
    	for(int Insi=0; Insi<Anchor_AoBcomp.length; Insi++) 
    	{
    		...
    		for(int Rowi = 0; Rowi < row ; Rowi++)
    		{
    			...
    			int Bcomp_i=0;
    			//Sub array iteration (connection phase discriptor)
    			while(AoBcomp_anch_p<(Anchor_AoBcomp[Insi])) // p iterating to the next sub-array!
    			{
    				//################################// This is within one row where Ii & Ri is given!
