...
public class Concept {
	 ...
	 * consider this:
	 * 
	 * This whole algorithm needs one big src array!
	 * float[] src -> C matrix of Matmul;
	 * 
	 * then there is a big array of nodes!
	 * some of these nodes are head nodes!
	 * 
	 * meaning: -> they derive the src array!!
	 * ... in practice there isn't always 
	 * a derivative for every head->src relationship (because they are not connected....)
	 * 
	 * ...but still:
	 * the algorithm needs that freedom in order to stay simple!
	 * 
	 * every node has a local head to input relationship
	 * ...and (possibly) a head to src relationship!
	 * 
	 * 
	 * */
