...
public class CPUSum implements ImplementationFor<CPU>
{
    ...
    private Tsr<?> _runRecursively(Tsr<?> in, CPU device)
    {
        ...
        if ( type == Float.class ) {
            float[] inData = in.getMut().getData().getRef(float[].class);
            ...
            double[] inData = in.getMut().getData().getRef(double[].class);
            ...
            int[] inData = in.getMut().getData().getRef(int[].class);
            int[] out = new int[N];
            executor.threaded( N, ni -> {
                int offset = ni * RTS;
                int limit = Math.min( offset + RTS, SIZE );
            ...
            long[] inData = in.getMut().getData().getRef(long[].class);
            long[] out = new long[N];
            executor.threaded( N, ni -> {
                int offset = ni * RTS;
                int limit = Math.min( offset + RTS, SIZE );
            ...
            short[] inData = in.getMut().getData().getRef(short[].class);
            ...
            byte[] inData = in.getMut().getData().getRef(byte[].class);
            byte[] out = new byte[N];
            executor.threaded( N, ni -> {
                int offset = ni * RTS;
                int limit = Math.min( offset + RTS, SIZE );
            ...
            Object[] inData = in.getMut().getData().getRef(Object[].class);
