 ...
 *  the component system for implementation instances of the {@link FunImplementationFor} interface.
...
implements DeviceAlgorithm<C>
{
    ...
    protected final Map<Class<Device<?>>, FunImplementationFor<Device<?>>> _implementations = new HashMap<>();

    public AbstractDeviceAlgorithm( String name ) { super( name ); }

    @Override
    public <D extends Device<?>, E extends FunImplementationFor<D>> C setImplementationFor(
        ...
        _implementations.put(
            ...
            (FunImplementationFor<Device<?>>) implementation
        );
        return (C) this;
    }

    @Override
    public <D extends Device<?>> FunImplementationFor<D> getImplementationFor( Class<D> deviceClass )
    {
        FunImplementationFor<D> found = (FunImplementationFor<D>) _implementations.get( deviceClass );
        if ( found == null )
            for ( Class<Device<?>> type : _implementations.keySet() )
                if ( type.isAssignableFrom(deviceClass) )
                    return (FunImplementationFor<D>) _implementations.get(type);
