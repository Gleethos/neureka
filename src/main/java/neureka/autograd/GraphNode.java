...
public class GraphNode implements Component
{
    ...
    private void _construct(Tsr output, Function function, Tsr[] inputs, GraphLock lock)
    {
        ...
        if(_nid==-1){
            long nid = 1;
            if(_parents !=null) {
                for(GraphNode n : _parents) nid*=n.getPayload().hashCode(); //payload might be 0! Why? -> garbage collected!
            }
    ...
    private void _backward(Tsr error, Set<GraphNode> pendingNodes, boolean allowPendingError)//Map<GraphNode, PendingError> pendingBackProp, boolean allowPendingError)
    {
        ...
        if(this.usesAD())
        {
            ...
            if(this.usesReverseAD()) this.forEachBackward(error, (t, e)->t._backward(e, pendingNodes, true));
    ...
    private void _backwardJIT(Tsr error, GraphNode source){
        ...
        if(this.usesAD()) {
            ...
            if(this.usesReverseAD()) this.forEachBackward(error, (t, e)->t._backwardJIT(e, source));
            //Standard reverse mode-AD:
    ...
    public void put(GraphNode target, Object o)
    {
        ...
        if(agent.isForward()){
            ...
            if(d!=null && d.has(GraphNode.class))((GraphNode)d.find(GraphNode.class))._is_used_as_derivative = true;
        }
