...
public class GraphNode implements Component {
    ...
    public boolean reliesOnJustInTimeProp() {
        return _relies_on_JIPProp > 0;
    }

    private int _relies_on_JIPProp = 0;
            ...
            if (this.usesForwardAD()) {
                for (int i = 0; i < inputs.length; i++) {
                    ...
                    if (src_node.function() != null && src_node.function().id() == Function.TYPES.LOOKUP.get("x")) {
                        this.put(src_node, function.getADAgent(inputs, i, true));//TODO: is this ever used? / visited? - yes but why?
                        ...
                        if (src_node.usesAD()) {
                            if (src_node.size() == 0 && this.size() == 0) {
                                ...
                                src_node.forEach(function.derive(inputs, i),
                                        (t, td) -> {
                                            if (this.has(t))
                                                this.put(t, ADD.activate(new Tsr[]{td, (Tsr) this.get(t)}));
                                            else this.put(t, td);
    ...
    private void _backwardJIT(Tsr error, GraphNode source) {
        if (getPayload().isOutsourced()) getPayload().device().add(error);
        if (getPayload().rqsGradient()) getPayload().addToGradient(error);
        if (this.reliesOnJustInTimeProp()) _relies_on_JIPProp--;
        else throw new IllegalStateException("[GraphNode][_backwardJIT]: JITProp backward on non JITPROP nodes!");
        ...
        if (_pending_error != null && source != this) {
            _pending_error.accumulate(error);
            /*
              A pending error has been found, so this means that this very node
              is referenced by one or more JIT-Prop components.
              If among these components is the one that has issued this very
              traverse we are in at this moment, then this pending error at this node will later on
              be continued to be propagated by another JITProp component.
