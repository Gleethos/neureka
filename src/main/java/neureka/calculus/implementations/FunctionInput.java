...
public class FunctionInput implements Function, GradientProvider
{
    ...
    private Tsr<?> _extract( Tsr<?> t )
    {
        if ( this.providesGradient() ) {
            ...
            if ( t.rqsGradient() ) {
                if ( gradient == null ) {
                    gradient = Tsr.of( t.getItemClass(), t.shape(), 0.0 );
                    t.set( (Tsr) gradient );
                }
                return gradient;
            }
    ...
    public Tsr<?> execute( Args arguments, Tsr<?>... inputs ) {
        ...
        if ( d >= 0 )
            return ( d == index() )
                ? Tsr.of( inputs[ 0 ].getItemClass(), inputs[ 0 ].shape(), 1.0 ).getUnsafe().setIsIntermediate( true )
                : Tsr.of( inputs[ 0 ].getItemClass(), inputs[ 0 ].shape(), 0.0 ).getUnsafe().setIsIntermediate( true );
