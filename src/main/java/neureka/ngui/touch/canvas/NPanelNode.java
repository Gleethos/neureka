...
public class NPanelNode implements NPanelObject
{
	...
	{
		Core = neuron;
		//neuron.asCore().overwrite64(this);
		position[0] = x;
		position[1] = y;
	...
	NPanelNode(double x, double y, int ID) 
	{
		...
		Core = new Tsr();
		Core.add(this);
		//Core.overwrite64(new NVOptimizer(Core.size(), Core.getConnection(), 3));
	...
	@Override
	public ArrayList<NPanelRepaintSpace> moveCircular(double[] data, TSurface_I Surface) //double centerX, double centerY, double x, double y
	{
	...
	private ArrayList<NPanelRepaintSpace> moveCircularBy(double[] data, TSurface_I Surface)
	{
	...
	private ArrayList<NPanelRepaintSpace> updateAnimations(TSurface_I HostPanel) {
	...
	// INTERACTION:
	// --------------------------------------------------------------------------------------------
	public void movementAt(double trueMouseX, double trueMouseY, TSurface_I HostSurface)
	...
	{
		if(Core.rqsGradient() && position.length==4)
		{
	...
	{//System.out.println("convection: "+convection);
		...
		//				   if(((PanelNode)connection[Ii][Ni].asCore().findModule(PanelNode.class)).justMoved()) {
		...
		//					if(connection[Ii][Ni]!=null)
		//				  	{
		//					    vX += ((PanelNode)connection[Ii][Ni].asCore().findModule(PanelNode.class)).getX() - getX();
		//				        vY += ((PanelNode)connection[Ii][Ni].asCore().findModule(PanelNode.class)).getY() - getY();
		//				    }
		//				  //...
		//				}
		//			if (vX == 0 && vY == 0)
		...
		//						 if (connection[Ii][Ni]!=null)
		//						    {
		//							 double connectionX = ((PanelNode)connection[Ii][Ni].asCore().findModule(PanelNode.class)).getX();
		//							 double connectionY = ((PanelNode)connection[Ii][Ni].asCore().findModule(PanelNode.class)).getY();
		//							 if(dataDisplayUpdateRequest) {
		...
		//								 //WEIGHT REPAINT:
		//								 queue.overwrite64(new NPanelRepaintSpace((midPX),(midPY), 50, 25));
		//							 	}
		...
		//						     queue.overwrite64(new NPanelRepaintSpace(
		//								repaintCenterX,
		//								repaintCenterY,
		//								distanceX,
		//								distanceY));
		...
		//				    		queue.overwrite64(InputNode.get(Ii).getRepaintSpace(diameter/2));
	...
	private void paintNodeConnections(Graphics2D brush,  NAnimator Animator) 
	{
		...
		//					if(activityCheck)
		//					{
		//						PanelNode Other = ((PanelNode)connection[Ii][Ni].asCore().findModule(PanelNode.class));
		//
		//						double d = Math.pow(Math.pow(vectorX, 2)+Math.pow(vectorY, 2), 0.5);
	...
	public void paintNeuron(Graphics2D brush, double panelScale, NAnimator Animator) 
	{
		...
		if (Core.isOutsourced() && Core.rqsGradient()) //Core.isLast() && Core.isFirst()
		{
			deckColor = new Color(140, 140, 0);
		}
		if (Core.rqsGradient()) //isParalyzed()
		{
		...
		if(panelScale>0.1)
		{
			...
			for (int i = 0; i < InputNode.size(); i++)
			{
				...
				.paintTop(
					...
					getY(),
					diameter / 2,
					"inputNodedispplay",//Formatter.format(Core.getInput(0, 0)),
					(Math.pow(mod, 6))
					);
	...
	private void paintNodeInterior(Graphics2D brush, int centerX, int centerY, double modifier, double panelScale, NAnimator Animator)
	{
		...
		brush.setColor(Color.CYAN);
		brush.drawString("Paralyzed: "+Core.rqsGradient()+";",
		...
		brush.setColor(Color.CYAN);
		brush.drawString("First: "+Core.rqsGradient()+"; Hidden: "+Core.rqsGradient()+"; Last: "+Core.rqsGradient()+";",
		...
		if (Core.rqsGradient())
		{
			...
			brush.drawString(""+Formatter.format(Core), (int) (centerX - 50), (int) (centerY - 210));
		}

		ArrayList<Painter> PainterArray = new ArrayList<Painter>();
		if(Core.has(NVControlFrame.class))
		{
			PainterArray.add
	...
	private void paintNodeDeck(Graphics2D brush, int centerX, int centerY, double modifier, double panelScale, NAnimator Animator)
	{
		...
		//brush.fillRect((int) (centerX - radius*0.1*Long.toString(Core.size()).length()), (int) (centerY -radius*0.25), (int)(2*radius*0.1*Long.toString(Core.getID()).length()), (int)(radius*0.02));
		//brush.setColor(Color.BLACK);
		brush.drawString(Long.toString(Core.size()),(int) (centerX - radius*0.038*Long.toString(Core.size()).length()), (int) (centerY -radius*0.25));//+radius*0.385
		//brush.drawString(Long.toString(Core.getID()),(int) (centerX - radius*0.0297*Long.toString(Core.getID()).length()), (int) (centerY +radius*0.385));
	...
	////@Override
	public void repaintLayer(int layerID, Graphics2D brush, TSurface_I HostSurface)
	{
		switch(layerID) 
		{
