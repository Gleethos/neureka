...
public class NCircleMenu implements NPanelObject{
	...
	public void drawMenu(GraphicsContext brush)
	{
		...
		if(primMod>0) {
			...
			Point2D center = new Point2D.Float((float)X, (float)Y);
				...
				Paint oldPaint = null;//brush.getPaint();
			...
			for(int i=0; i<mapping.length; i++) {
				...
				////oldPaint = brush.getPaint();
				 ...
				 Graphics2D b = null;
				 drawMenuIcon(mapping[i],ovalX, ovalY, b);
			}
			////brush.setClip(null);
			...
			if(highlightID!=0) {
				 ...
				 brush.fillOval((int)(ovalX-ovalRadius*0.99), (int)(ovalY-ovalRadius*0.99), (int)(2*ovalRadius*0.98), (int)(2*ovalRadius*0.98));
				 Graphics2D b = null;
				 drawMenuIcon(mapping[highlightID-1],ovalX, ovalY, b);
			}
		 }		
	}
	...
	public ArrayList<NPanelRepaintSpace> updateOn(TSurface_I HostPanel)
	{
		...
		Object something = ((TSurface)HostPanel).findAnything(X, Y, true, this);
		...
		if(PropertyContainer instanceof NCircleMenu)
		{
			...
			if(animationCounter<CounterLimit) 
			{
				...
				{ 
					...
					((TSurface)HostPanel).translatePanel(-(shiftX*HostPanel.getScale()),-(shiftY*HostPanel.getScale()));
					X += shiftX;
					Y += shiftY;
				}
			...
			{
				((TSurface)HostPanel).scaleAtReal(X, Y, (double)(1-(1000/(double)frameDelta)));
		...
		{
			if(PropertyContainer instanceof TSurface)
			{
			...
			{//-----------------------------------------------------------------------
				...
				else if(chosen==5) //inner node setting!
				{
					...
					Actor = (int choice, Object Container)->{
						if(Container instanceof TSurface) {
						    TSurface panel = (TSurface)Container;
						    //Setting node accordingly
						}
					};
				}
				else if(chosen==9) //Node _id menu
				{
					animationCounter=-2*CounterLimit;
	...
	//----------------------------------------------------------
	@Override
	public void movementAt(double x, double y, TSurface_I HostSurface) {
		if(mapping==null) {return;}
	...
	@Override
	public boolean needsRepaintOnLayer(int layerID) {if(this.getLayer().getOrder()==layerID) {return true;}return false;}
	@Override
