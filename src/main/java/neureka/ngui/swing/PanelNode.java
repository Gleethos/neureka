...
public class PanelNode extends Node implements NPanelObject
{
	...
	//===========================================================================
	//===========================================================================
	PanelNode(double x, double y, int ID)
	{
		position[0] = x;
		position[1] = y;
		InputNode = new ArrayList<NPanelNodeInput>();
		...
		Core.add(this);
		this.setTensor(Core);
		//this.getTensor().overwrite64(new NVOptimizer(this.getTensor().size(), this.getTensor().getConnection(), 3));
	...
	public void addInputNode() 
	{
		...
		//System.out.println("New connection input size: "+this.getTensor().inputSize());
			...
			if(this.getTensor().rqsGradient()) {
				if(position.length==4) 
				{
					...
					if(distance < 2.5*diameter) {
						...
						//		if(RimNodes[Ni]!=null) {
						//			PanelNode panelCompartment = (PanelNode) RimNodes[Ni].asCore().findModule(PanelNode.class);
	...
	public ArrayList<NPanelRepaintSpace> updateOn(NPanel_I HostPanel)
	{
		...
		//		    	if(RimNodes[Ni]!=null)
		//		        {
		//		           PanelNode panelNode = (PanelNode) RimNodes[Ni].asCore().findModule(PanelNode.class);
		//		           if(panelNode!=null)
		...
		//		        	   				else
		//		        	   				{
		//		        	   				 queue.overwrite64(panelNode.getRepaintSpace());
		...
		//		    }
		//}
		//Gtting input node rotations!
		//-adding them together!
		//dawing a core body accordingly!
		//-sizing input nodes according to core connections...
		//coosing coloring.... Suggestion: ?
		}
		else
		{
			if(position.length!=2) {
	...
	private ArrayList<NPanelRepaintSpace> updateAnimations(NPanel_I HostPanel) {
			...
			for(int Ii=0; Ii<InputNode.size(); Ii++) 
			    {
				 	...
				 	//System.out.println("Value activity found... => convection");
				 	Ii=InputNode.size();}
				}
	...
	// INTERACTION:
	// --------------------------------------------------------------------------------------------
	public void movementAt(double trueMouseX, double trueMouseY, NPanel_I HostSurface)
	...
	{
		if(this.getTensor().rqsGradient() && position.length==4)
		{
	...
	{//System.out.println("convection: "+convection);
		...
		double vY = 0;
		ArrayList<NPanelRepaintSpace> queue = new ArrayList<NPanelRepaintSpace>();;
		ArrayList<Node> connection = this.getConnection();
		if (connection != null && connection.size()>0)
			...
			for (int Ii = 0; Ii < connection.size(); Ii++)
			{
				...
				if (connection.get(Ii) != null && connectionMoved==false)
				{
					//for (int Ni = 0; Ni < connection.get(Ii).size(); Ni++)
					//{
						//if(connection.get(Ii)!=null) {
							//if(this.getNeuron().isBasicRootNode()) {System.out.println("Bananas baby");}
						   if(((PanelNode)connection.get(Ii)).justMoved()) {
					...
					{//=================================================================
					 ...
					 if(connection.get(Ii)!=null) {
						...
						//  	{
							    vX += ((PanelNode)connection.get(Ii)).getX() - getX();
						        vY += ((PanelNode)connection.get(Ii)).getY() - getY();
						//    }
						//  //...
						//}
					if (vX == 0 && vY == 0)
					...
					else
					{
						...
						if (InputNode.get(Ii).changeOccured() || convection) {
						    //for (int Ni = 0; Ni < connection.get(Ii).length; Ni++) {
								 ...
								 //if (connection[Ii][Ni]!=null)
								 //   {
									 double connectionX = ((PanelNode)connection.get(Ii)).getX();
									 double connectionY = ((PanelNode)connection.get(Ii)).getY();
									 if(dataDisplayUpdateRequest) {
	...
	private void paintNodeConnections(Graphics2D brush,  NAnimator Animator) 
	{
		...
		//=======================================================================================================================
		ArrayList<Node> connection = getConnection();
		if (connection != null)
		{
			for (int Ii = 0; Ii < connection.size(); Ii++)
			{
				if (connection.get(Ii) != null)
				{
					...
					double inputVecY = selfCenterY - getY();
							...
							if(activityCheck)
							{
								PanelNode Other = ((PanelNode)connection.get(Ii));
							...
							//if(this.this.getTensor().hasWeight(Ii))
							//{
								String value = ""+ this.getTensor();
	...
	public void paintNeuron(Graphics2D brush, double panelScale, NAnimator Animator) 
	{
		...
		if (this.getTensor().isOutsourced() && this.getTensor().rqsGradient()) //this.getTensor().isLast() && this.getTensor().isFirst()
		{
			deckColor = new Color(140, 140, 0);
		}
		if (this.getTensor().rqsGradient()) //isParalyzed()
		{
		...
		else if(this.getTensor().rqsGradient())
		{
	...
	private void paintNodeInterior(Graphics2D brush, int centerX, int centerY, double modifier, double panelScale, NAnimator Animator) 
	{
		...
		brush.setColor(Color.BLACK);
		brush.drawString(Formatter.format(Tsr.fcn.io.getFrom(this.getTensor(), 0)), (int) (centerX - radius*0.108), (int) (centerY +radius*0.0475- radius*0.260*buttonAnimationModifier));
		
		ValueFont = new Font("Tahoma", Font.PLAIN, (int)(diameter*(0.033333)));
		brush.setFont(ValueFont);
		
		brush.setColor(Color.CYAN);
		brush.drawString("Paralyzed: "+this.getTensor().rqsGradient()+";",
		...
		brush.setColor(Color.CYAN);
		brush.drawString("First: "+this.getTensor().rqsGradient()+"; Hidden: "+this.getTensor().rqsGradient()+"; Last: "+this.getTensor().rqsGradient()+";",
		...
		//brush.drawString("AutoApply: "+this.getTensor().isInstantGradientApply()+";", (int) (centerX - radius*0.825), (int) (centerY -radius*0.0));
		//brush.drawString("Summing: "+this.getTensor().isGradientSumming()+";",        (int) (centerX - radius*0.825), (int) (centerY +radius*0.1));
		brush.setColor(Color.DARK_GRAY);
		...
		//brush.drawString("Bias: "+this.getTensor().biasIsTrainable()+"; ",    (int) (centerX + radius*0.425), (int) (centerY -radius*0.0));
		//brush.drawString("Weight: "+this.getTensor().asCore().weightIsTrainable()+";",  (int) (centerX + radius*0.4), (int) (centerY +radius*0.1));
		...
		brush.setFont(ValueFont);
		brush.setColor(Color.DARK_GRAY);
		//brush.fillRect((int) (centerX - radius*0.1*Long.toString(this.getTensor().getID()).length()), (int) (centerY +radius*0.385), (int)(2*radius*0.1*Long.toString(this.getTensor().getID()).length()), (int)(radius*0.02));

		brush.setColor(Color.CYAN);
		//brush.drawString(Long.toString(this.getTensor().getID()),(int) (centerX - radius*0.0297*Long.toString(this.getTensor().getID()).length()), (int) (centerY +radius*0.385));
		//brush.drawString(Long.toString(this.getTensor().getID()),(int) (centerX - radius*0.02927*Long.toString(this.getTensor().getID()).length()), (int) (centerY +radius*0.385));
		...
		//NVCloak Public = (NVCloak) this.getTensor().find(NVCloak.class);
		//if(this.getTensor().getFunction()!=null)
		//{
		//	listedProperties += this.getTensor().getFunction().toString()+"";
		//}
		//NVMemory Memory = (NVMemory) this.getTensor().findModule(NVMemory.class);
		//if(Memory!=null)
		//{
		...
		if (this.getTensor().rqsGradient()) {
			...
			brush.drawString(""+Formatter.format(this.getTensor()), (int) (centerX - 50), (int) (centerY - 210));
		}

		ArrayList<Painter> PainterArray = new ArrayList<Painter>();
		if(this.getTensor().has(NVControlFrame.class))
		{
			PainterArray.add
	...
	private void paintNodeDeck(Graphics2D brush, int centerX, int centerY, double modifier, double panelScale, NAnimator Animator) 
	{
		...
		//brush.fillRect((int) (centerX - radius*0.1*Long.toString(this.getTensor().size()).length()), (int) (centerY -radius*0.25), (int)(2*radius*0.1*Long.toString(this.getTensor().getID()).length()), (int)(radius*0.02));
		//brush.setColor(Color.BLACK);
		brush.drawString(Long.toString(this.getTensor().size()),(int) (centerX - radius*0.038*Long.toString(this.getTensor().size()).length()), (int) (centerY -radius*0.25));//+radius*0.385
		//brush.drawString(Long.toString(this.getTensor().getID()),(int) (centerX - radius*0.0297*Long.toString(this.getTensor().getID()).length()), (int) (centerY +radius*0.385));
		...
		brush.setFont(ValueFont);
		String acti = Formatter.format(Tsr.fcn.io.getFrom(this.getTensor(), 0));
		...
		{
			brush.drawString("Optimum:", (int) (centerX - 50), (int) (centerY - 240));
			brush.drawString(""+Formatter.format(Tsr.fcn.io.getFrom(this.getTensor(), 0)), (int) (centerX - 25), (int) (centerY - 210));
		}
		brush.setClip(null);
	}
	...
	public boolean needsRepaintOnLayer(int layerID) 
	{
		if(layerID==0) 
		{
			if(this.getTensor().rqsGradient()==false)
			{
				return true;
			}
			return false;
		...
		if(layerID==2) 
		{
			if(this.getTensor().rqsGradient()==true)
			{
				return true;
			}
			return false;
