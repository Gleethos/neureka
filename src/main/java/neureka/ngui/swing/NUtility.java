...
public double[] getCurvePointOn(double t, double[] point) {
	...
	if(point.length%2==1) {return null;}
...
public int choose(int n, int k) {
		...
		int nCk = 1;
		for(int ki=0; ki<k; ki++) {nCk = nCk * (n-ki) / (ki+1);}
		return nCk;
}

public double lengthOf(double vecX,  double vecY) {return Math.pow(Math.pow(vecX, 2)+Math.pow(vecY, 2), 0.5);}

public double unitaryVectorProduct(double firstX, double firstY, double secX, double secY) 
{
	double d =  lengthOf(firstX, firstY)*lengthOf(secX, secY);
