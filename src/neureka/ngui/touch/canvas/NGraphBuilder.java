...
public class NGraphBuilder {
	...
	private void addPanelTensor(int x, int y)
 	...
 	private void addPanelTensor(NPanelNode PanelNeuron)
 	{
 		if(Surface.getMap()==null)
 		{
	...
	public NGraphBuilder()
	{
		...
		NPanelNode Neuron = new NPanelNode(-400, 2500, 0);
	 	addPanelTensor(Neuron);
	 	NPanelNode Neuron1 = new NPanelNode( 700, -2600, 1 );
				...
				if(newMenu==null) //TODO: Presspoint sction on surface!
				{
					...
					Surface.getMap().applyToAllWithin(frame,
						(NPanelObject thing)->
						{
						    ...
						    {
						    	if(thing.getLayer().getOrder()>=newMenu.getLayer().getOrder())
								{
						    		newMenu.setLayerID(thing.getLayer().getOrder()+1);
						    		System.out.println("mlid: "+newMenu.getLayer().getOrder());
						    		System.out.println("tlid: "+thing.getLayer().getOrder());
								}
						    }
						    return false;
						}
		...
		Surface.setSwipeAction(
			(surface)->
			{
				...
				if(surface.isInTouchMode())
				{
					...
					if(surface.getFocusObject()!=null)
					{
						ArrayList<NPanelRepaintSpace> RepaintQueue = null;//surface.getRepaintQueue();
	...
	//-------------------------------------------------------------------------
	 public void setHistoryScopeTo(int scope)
	 {
	 	...
	 	(NPanelObject node)->
	 	{
	 		if(!(node instanceof NPanelNode)) {return false;}
		 return true;
		 });
	 }
	 //-------------------------------------------------------------------------
