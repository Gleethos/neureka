...
import neureka.main.core.modul.calc.comp.Constant;
import neureka.main.core.modul.calc.comp.Input;
import neureka.main.core.modul.calc.comp.Variable;
...
public class FunctionFactory {
    ...
    private Function construct(String expression, boolean tipReached)
    {
        ...
        //===
        int Count = FunctionFactory.register.length;
        for (int j = FunctionFactory.register.length; j > 0; --j) {
            if (!utility.containsOperation(FunctionFactory.register[j - 1], Operations)) {
                --Count;
            } else {
                j = 0;
            }
        ...
        while (ID < Count) {
            ...
            if (utility.containsOperation(FunctionFactory.register[ID], Operations)) {
                ...
                if (enoughtPresent) {
                    ...
                    for (int Ci = 0; Ci < length; Ci++)
                    {
                        ...
                        if (currentOperation != null) {
                            if (currentOperation.equals(FunctionFactory.register[ID])) {
                                final String newChain =
                                        utility.groupBy(FunctionFactory.register[ID], currentChain, currentComponent, currentOperation);
            ...
            System.out.println("ID:" + ID + " => register[ID]: " + FunctionFactory.register[ID]);
        ...
        if (Operations.size() >= 1) {
            ...
            for (int k = 0; k < FunctionFactory.register.length; ++k) {
                if (FunctionFactory.register[k].equals(Operations.get(0))) {
                    f_id = k;
                }
            }
        }
    ...
    * */
    ...
    private static class utility {
        ...
        public static int numberOfOperationsWithin(final List<String> operations) {
            int Count = 0;
            for (int i = 0; i < FunctionFactory.register.length; ++i) {
                if (utility.containsOperation(FunctionFactory.register[i], operations)) {
                    ++Count;
                }
            }
            return Count;
        ...
        public static boolean isBasicOperation(final String operation) {
            ...
            for (int i = 0; i < FunctionFactory.register.length; ++i) {
                ...
                if (FunctionFactory.register[i].equals(operation)) {
                    System.out.println("");
                    return true;
                }
            }
