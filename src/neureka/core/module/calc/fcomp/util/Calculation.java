...
public class Calculation {
    ...
    public static T tensorActivationOf(T input, int f_id, boolean derive, boolean isFlat) {
        ...
        if(!derive){
            ...
            //output.addModule(new GraphBuilder(output, new T[]{input}, f_id, false, true));//(isFlat)?(!tipReached)?true:false:false));
        }
        return output;
    }
        ...
        if(d<0 && !isFlat){//implies !tipReached ==true // only flat functions can be executed
            if(f_id<=9){
                ...
                if(Context.REGISTER[f_id].length()!=1){
                    ...
                    //output.addModule(new GraphBuilder(output, tsrs, operation, true, false));
                    return output;
                }
            }
        }
