...
public class NVFHead implements NVFunction
{
    ...
    static 
    {
        ...
        {"relu", "sig", "tanh", "quad", "lig", "lin", 
         "gaus", "sum", "prod", "abs", "sin", "cos",
         "^",     "/",  "*",     "%",   "-",   "+"};
    }
    /*
    ...
    public NVFunction newBuild(String expression) 
    {
        ...
        expression = this.unpackAndCorrect(expression);
        ...
        while (i < expression.length()) 
        {
            ...
            final String newComponent = this.parsedComponent(expression, i);
            if (newComponent != null) 
            {
            	System.out.println("Component found! : " + newComponent + " ");
            	System.out.println(Components.size() +" < "+Operations.size()+" ?: true -> Components.add("+newComponent+")");
                ...
                final String newOperation = this.parsedOperation(expression, i);
        ...
        {
            if (!this.containsOperation(NVFHead.OperationRegister[j - 1], Operations)) 
            {
                --Count;
            }
            else 
        ...
        while (ID < Count) 
        {
            ...
            if (this.containsOperation(NVFHead.OperationRegister[ID], Operations))
            {
                
                String currentChain = null;
                boolean groupingOccured = false;
                boolean enoughtPresent = this.amountOfOperationsWithin(Operations) > 1;// Otherwise: I[j]^4 goes nuts!
                if(enoughtPresent)
                {
                	...
                	//while (ComponentIterator.hasNext())
	                {
	                    ...
	                    if (currentOperation != null) 
	                    {
	                        if (currentOperation.equals(NVFHead.OperationRegister[ID])) 
	                        {
	                            ...
	                            this.groupBy(NVFHead.OperationRegister[ID], currentChain, currentComponent, currentOperation);
        ...
        NFOperation[] OperationCollection = 
            {
            	...
            	new NFOperation() 
            	{
            		...
            		public double activate(final double[] input, int j) 
            		{
            		    ...
            		    if(nothingDone) {return Variable.get(0).activate(input);}
            		    return sum;
            		}
            		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            		@Override
            	...
            	new NFOperation() 
            	{
            		...
            		public double activate(final double[] input, int j) 
            		{
            		    ...
            		    if(nothingDone) {return Variable.get(0).activate(input, j);}
            		    return prod;
            		}
            		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            		@Override
        ...
        if (Components.size() == 1) 
        {
        	...
        	String possibleFunction = this.parsedOperation(Components.get(0).toLowerCase(), 0);
        		...
        		if(possibleFunction.length()>1) 
        		{
        			for(int Oi=0; Oi<OperationRegister.length; Oi++) {
        				if(OperationRegister[Oi].equals(possibleFunction)) 
        				{
        					...
        					newCore = newCore.newBuild(parsedComponent(Components.get(0),possibleFunction.length()));
            ...
            String component = this.unpackAndCorrect(Components.get(0));
            ...
            if (component.charAt(0) == 'i' || component.charAt(0) == 'I') 
            {
                ...
                newFunction = newFunction.newBuild(component);
                System.out.println("Input leave! -> return newInputLeave.newBuilt(component)");
                return newFunction;
            }
            System.out.println("Component is not of type Leave! -> component = cleanedHeadAndTail(component); ");
            //If the component did not trigger variable creation: Cleaning!
            component = cleanedHeadAndTail(component);
        ...
        else 
        {
            ...
            while (ComponentIterator2.hasNext()) 
            {
                ...
                System.out.println("this.Variable.add(newCore2.newBuild("+currentComponent2+")); Variable.size(): " + this.Variable.size());
    ...
    private int amountOfOperationsWithin(final List<String> operations) 
    {
        int Count = 0;
        for (int i = 0; i < NVFHead.OperationRegister.length; ++i) 
        {
            if (this.containsOperation(NVFHead.OperationRegister[i], operations)) 
            {
                ++Count;
            }
        }
    ...
    private String unpackAndCorrect(String equation) 
    {
        ...
        while (parsing && equation.charAt(0) == '(' && equation.charAt(equation.length() - 1) == ')') {
            ...
            if (needsStitching) {
                equation = this.removeHeadAndTail(equation);
            }
            else {
                parsing = false;
            }
    ...
    private String cleanedHeadAndTail(String exp) 
    {
    	System.out.println("Unclean component: "+exp);
        int Ci=0; String Updated=""; boolean condition=true;
        while(condition) {
        	if(isWeired(exp.charAt(Ci))||(exp.charAt(Ci)>='A'&&exp.charAt(Ci)<='Z')||(exp.charAt(Ci)>='a'&&exp.charAt(Ci)<='z'))
        	   {System.out.print("C: "+exp.charAt(Ci)+"; ");Ci++;}
        	else
        	   {condition=false;}
        	if(Ci==exp.length()) {condition=false;}
        }
        for(int Gi=Ci; Gi<exp.length(); Gi++) {Updated+=exp.charAt(Gi);}
        exp=Updated; 
        Updated="";System.out.print("\nUpdated: "+exp+"  \n");
        if(exp.length()>0) {
        	Ci=0;  condition=true; int l = exp.length()-1;
        	while(condition) { 
        	if(isWeired(exp.charAt(Ci))||(exp.charAt(l-Ci)>='A'&&exp.charAt(l-Ci)<='Z')||(exp.charAt(l-Ci)>='a'&&exp.charAt(l-Ci)<='z'))
        	   {System.out.print("C: "+exp.charAt(l-Ci)+"; ");Ci++;}
        	else
        	   {condition=false;}
        	if(l-Ci<0) {condition=false;}
        	}
        	for(int Gi=0; Gi<=l-Ci; Gi++) {Updated+=exp.charAt(Gi);}
        	exp=Updated;
        }
        if(exp.length()>0) {
        	if(exp.charAt(0)=='('&&exp.charAt(exp.length()-1)!=')') {exp=removeHeadAndTail(exp);}
        	if(exp.charAt(exp.length()-1)==')'&&exp.charAt(0)!='(') {exp=removeHeadAndTail(exp);}
        }
        System.out.println("Cleaned component: "+exp);
    	return exp;
    }
    
    private String removeHeadAndTail(final String equation) 
    {
        String corrected = "";
    ...
    private boolean isWeired(char c) 
    {
    	if(c=='"') {return true;}
    	if('§' == c) {return true;}
    	if(c=='$') {return true;}
    ...
    private String parsedOperation(final String equation, final int i) 
    {
        String operation = "";
    ...
    private String parsedComponent(final String equation, final int i) 
    {
        if (equation.length() <= i) 
        {
            return null;
        ...
        for (int Ei = i; Ei < equation.length(); ++Ei) {
            ...
            if (bracketDepth == 0) {
                ...
                for (int Sii = Ei + 1; Sii < equation.length(); ++Sii) {
                    ...
                    if (this.isBasicOperation(possibleOperation)) {
    ...
    private boolean isBasicOperation(final String operation) {
    ...
    private boolean containsOperation(final String operation, final List<String> operations) {
    ...
    private String groupBy(final String operation, final String currentChain, final String currentComponent, final String currentOperation) {
        String group = null;
        if (currentOperation != null) {
            if (currentOperation.equals(operation)) {
  ...
  //============================================================================================================================================================================================	
  	...
  	public double getReLuOf(double input, boolean derive)
  	{
  		...
  		else {return getReLuDerivativeOf(input);}
  	}
  	public double getReLuOf                     (double Input) {double Output; if(Input>=0){Output=(Input+shift)*inclination;}else{Output=(Input+shift)*secondaryInclination;} return Output;}
  	public double getReLuDerivativeOf           (double Input) {double Output; if(Input>=0){Output=inclination;}else{Output=secondaryInclination;} return Output;}
    //--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
  	public double getSigmoidOf(double input, boolean derive)
  	{
  		if(!derive)
		{
  		...
  		else {return getSigmoidDerivativeOf(input);}
  	}
  	public double getSigmoidOf           (double Input) {double Output=1/(1+Math.pow(Math.E, (-(Input+shift)*inclination))); return Output;}
  	public double getSigmoidDerivativeOf (double Input) {double Output=inclination*(Math.pow(Math.E, -(Input+shift)*inclination))/(Math.pow((1+Math.pow(Math.E, -(Input+shift)*inclination)),2)+2*Math.pow(Math.E, -(Input+shift)*inclination)); return Output;}
    //--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
  	public double getTanhOf(double input, boolean derive)
  	{
  		if(!derive)
		{
  		...
  		else {return getTanhDerivativeOf(input);}
  	}
  	public double getTanhOf              (double Input) {double Output=((Input+shift)*inclination)/Math.pow((1+Math.pow(((Input+shift)*inclination), 2)), 0.5); return Output;}
  	public double getTanhDerivativeOf    (double Input) {double Output=(1-Math.pow(((Input+shift)/Math.pow((1+Math.pow((Input+shift), 2)), 0.5)), 2))*inclination; return Output;}
    //--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
  	public double getQuadraticOf(double input, boolean derive)
  	{
  		if(!derive)
		{
  		...
  		else {return getQuadraticDerivativeOf(input);}
  	}
  	public double getQuadraticOf              (double Input) {double Output=((Input+shift)*(Input+shift)*inclination); return Output;}
  	public double getQuadraticDerivativeOf    (double Input) {double Output=2*Input*inclination+2*shift*inclination; return Output;}
    //--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
  	public double getLigmoidOf(double input, boolean derive)
  	{
  		if(!derive)
		{
  		...
  		else {return getLigmoidDerivativeOf(input);}
  	}
  	public double getLigmoidOf              (double Input) {double Output=(inclination*(Input+shift)+(Math.log(Math.pow(Math.E, -(Input+shift)*inclination)+1)/Math.log(Math.E))); return Output;}
  	public double getLigmoidDerivativeOf    (double Input) {double Output=inclination/(1+Math.pow(Math.E, ((-Input+shift)*inclination))); return Output;}
    //--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
  	public double getLinearOf(double input, boolean derive)
  	{
  		if(!derive)
		{
  		...
  		else {return getLinearDerivativeOf(input);}
  	}
  	public double getLinearOf              (double Input) {double Output=inclination*(Input+shift); return Output;}
  	public double getLinearDerivativeOf    (double Input) {double Output=inclination; return Output;}
  //--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
  	public double getGaussianOf(double input, boolean derive)
  	{
  		if(!derive)
		{
  		...
  		else {return getGaussianDerivativeOf(input);}
  	}
  	public double getGaussianOf              (double Input) {double Output=Math.pow(Math.E, -Math.pow(inclination*(Input+shift),2)); return Output;}
  	public double getGaussianDerivativeOf    (double Input) {double Output=-2*(inclination*(Input+shift))*Math.pow(Math.E, -Math.pow(inclination*(Input+shift),2)); return Output;}
  //--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------							
