...
__kernel void power_broadcast_inv(
    ...
    int d
        ...
        if(d<0){
            drn[_i_of_i(i, prv_drn_cfg, rank)] =
                    pow(value, src1[_i_of_i(i, prv_src1_cfg, rank)]);
        } else {
            if(d==0){
                drn[_i_of_i(i, prv_drn_cfg, rank)] =
                    src1[_i_of_i(i, prv_src1_cfg, rank)]*pow(value, src1[_i_of_i(i, prv_src1_cfg, rank)]-(float)1);
            } else {
                drn[_i_of_i(i, prv_drn_cfg, rank)] =
                    pow(value, src1[_i_of_i(i, prv_src1_cfg, rank)])
                        *log(value);
